...
public class SkuServiceImpl implements SkuService {
    ...
    public List<SkuVo> getBySpuIdWithAttributes(Long spuId) {
        ...
        // Step1: 读缓存
        List<SkuVo> result = redisUtils.getObject(key,
                new com.fasterxml.jackson.core.type.TypeReference<List<SkuVo>>() {});
        if (result != null) {
        ...
        // Step2: 缓存未命中，查库组装
        result = doGetBySpuIdWithAttributes(spuId);
        if (result == null) {
    ...
    public List<SkuStoreVo> getStoreBySpuIdWithAttributes(Long spuId) {
        ...
        List<SkuStoreVo> cached = redisUtils.getObject(key,
                new com.fasterxml.jackson.core.type.TypeReference<List<SkuStoreVo>>() {});
        if (cached != null) {
            return cached;
        }
    ...
    public List<SkuAdminVo> getAdminBySpuIdWithAttributes(Long spuId) {
        ...
        List<SkuAdminVo> cached = redisUtils.getObject(key,
                new com.fasterxml.jackson.core.type.TypeReference<List<SkuAdminVo>>() {});
        if (cached != null) {
            return cached;
        }
    ...
    private void fillInStock(List<SkuVo> skus) {
        ...
        for (SkuVo vo : skus) {
            ...
            String stockVal = redisUtils.get(stockKey);
            String frozenVal = redisUtils.get(frozenKey);
            try {
    ...
    private void cacheResult(String key, Object value, long ttlMinutes) {
        redisUtils.setObject(key, value, ttlMinutes, TimeUnit.MINUTES);
    }

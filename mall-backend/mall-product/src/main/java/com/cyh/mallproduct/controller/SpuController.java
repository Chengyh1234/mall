...
import com.cyh.mallproduct.dto.SpuDto;
...
public class SpuController {
    ...
    public Result<Map<String, Object>> add(@RequestPart(value = "spuDto") String spuDtoString,
        ...
        // 2. 根据商家ID自动获取其所属店铺ID
        Result<StoreInfoVo> storeResult = storeClient.getStoreBySeller();
        if (storeResult == null || storeResult.getData() == null) {
            return Result.error("未找到您的店铺，请先创建店铺");
        }
        StoreInfoVo storeInfo = storeResult.getData();
        ...
        spu.setStoreId(storeInfo.getId());          // 自动设置店铺ID
    ...
    public Result<SpuDetailVo> getById(@PathVariable Long id) {
        // cyhcanupdate z这里的spuDetailVo不对，应该不反悔商家信息，而是返回店铺的
        Spu spu = spuService.getById(id);
        if (spu == null) {
            return Result.error("商品不存在");
        }

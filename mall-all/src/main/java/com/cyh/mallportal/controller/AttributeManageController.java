...
public class AttributeManageController {
    ...
    @PreAuthorize("hasRole('SUPER_ADMIN') or hasRole('ADMIN')")
    public Result<List<AttributeValue>> listAttributeValues(@PathVariable Long attrId) {
        List<AttributeValue> list = attributeManageService.listAttributeValues(attrId);
        return Result.success(list);
    }
}
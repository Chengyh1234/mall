...
public class CategoryAttributeManageController {
     ...
     * 前端可根据该字段展示锁定图标，提示运营管理员哪些属性无法解绑
     *
    ...
    @PreAuthorize("hasRole('SUPER_ADMIN') or hasRole('ADMIN')")
    public Result<List<Attribute>> getAvailableAttributes(@PathVariable Long categoryId) {
        List<Attribute> list = categoryAttributeManageService.getAvailableAttributes(categoryId);
        return Result.success(list);
    }
